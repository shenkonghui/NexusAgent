...
// localStorage key：顶层任务模式（历史上有 coding/docs，现仅 coding）
const TASK_MODE_KEY = 'opennexus.taskMode'
...
export default function ChatPage() {
  ...
  // 历史上有 coding/docs 两种模式，docs 模式已移除；老用户 localStorage 残留的 'docs'
  // 在初始化时一次性迁移为 'coding'，避免无意义的状态。
  const [taskMode, setTaskMode] = useState<TaskMode>(() => {
    const saved = localStorage.getItem(TASK_MODE_KEY)
    if (saved !== 'coding') {
  ...
  const handleNewTask = useCallback(() => {
    navigate(newTaskUrl(workspaceId))
          ...
          <div className={styles.header}>
            <div className={styles.sysBar}>
              ...
              <div className={styles.actions}>
                ...
                <button type="button" className={styles.newTaskBtn} onClick={handleNewTask}>
        ...
        <AppLayout sidebarProps={{ sessions, workspaceId, onDelete: handleDeleteSession, onRename: handleRenameSession }}>
          <div className={styles.main}>
            <div className={`${styles.header} ${styles.headerSingle}`}>
              ...
              <div className={styles.actions}>
                <WorkspaceSelector value={workspaceId} onChange={handleWorkspaceChange} onRefresh={handleWorkspaceRefresh} onError={setError} />
                <button type="button" className={styles.newTaskBtn} onClick={handleNewTask}><Plus size={14} style={{ verticalAlign: '-2px' }} /> {t('session.newSession')}</button>
                <UserMenu />
              </div>
            </div>
