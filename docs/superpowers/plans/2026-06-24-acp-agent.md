...
**架构：** 厚 P1（`internal/acp`：协议封装+会话持久化+工作区）+ 薄 P2（`internal/agent`：注册表+路由）。复用 P3 的 config/database/models/repository 分层。模块名 `opennexus`，导入路径 `opennexus/internal/...`。
...
**包名约定：** 目录名作包名（`acp`、`agent`）。模块名 `opennexus`，导入路径 `opennexus/internal/...`。
...
SERVER_PORT=18080 \
DATABASE_PATH="./data/opennexus.db" \
go run ./cmd/server
```
预期：日志输出 `openNexus 认证服务启动于 :18080`，无 P1/P2 装配错误。
