# ~/.nextAgent 数据目录统一 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 将数据库、临时会话工作区、配置文件默认统一到 `~/.nextAgent/`，使本地启动与 Mac/Electron 安装版数据互通。

**Architecture:** 后端解析配置路径顺序为 `CONFIG_PATH` → `~/.nextAgent/config.yaml` → `./config.yaml`；`Validate` 将空的 `database.path` 默认设为 `~/.nextAgent/nexus.db`；桌面启动器 `--data-dir` 改为 `$HOME/.nextAgent`。Docker 仍用环境变量，不改。

**Tech Stack:** Go、config.yaml、shell 启动脚本、Electron main.cjs

## Global Constraints

- 配置优先序：`CONFIG_PATH` → `~/.nextAgent/config.yaml` → `./config.yaml`
- 默认数据根：`~/.nextAgent/`（nexus.db、session/）
- 不做旧数据自动迁移
- Docker 路径保持环境变量覆盖
- 不主动 commit（除非用户要求）
...
- Produces: `Validate` 后 `Database.Path` 默认为 `~/.nextAgent/nexus.db`
...
并将 `config.yaml` 中 `database.path` 改为空或注释说明默认 `~/.nextAgent/nexus.db`（若保留显式值，开发时仍会覆盖默认——推荐改为注释掉 path 或写 `~/.nextAgent/nexus.db` 并在 Load 后 expandPath）。

推荐：`config.yaml` 写 `path: ~/.nextAgent/nexus.db`，在 `Validate` 或 `applyEnv` 后对 database.path 做 `expandPath`。
...
启动器：仅当 `~/.nextAgent/config.yaml` 不存在时，再 `export CONFIG_PATH` 指向 bundle/旁路默认配置；若用户配置存在则不设 `CONFIG_PATH`。
...
`startBackend` 中：若 `~/.nextAgent/config.yaml` 存在则不设 `CONFIG_PATH`，否则设为 bundle/项目 config。
